...
const regForPassword=RegExp(/^(?=.[0-9])(?!.?\d{3})(?=.[a-zA-Z])(?!.?[a-zA-Z]{3})(?=.[~!@#$%^&()+-?])([a-zA-Z0-9~!@#$%^&*()+-?]{8,})$/);
let pass;
export class Validation extends Component {
    ...
    handler = (event) => {
        ...
        switch (name) {
            ...
            case 'age':
                errors.age = (regForAge.test(value)) && (value <= 100) ? '' : '! age Should contain number ';
            case 'email':
                errors.email = regForEmail.test(value) ? '' : '! Email is not valid';
                break;
            case 'password':
                pass=value;
                errors.password =(regForPassword.test(value)) && (value.length < 8 )? '! Password must me 8 chanrater long' : '';
                break;
